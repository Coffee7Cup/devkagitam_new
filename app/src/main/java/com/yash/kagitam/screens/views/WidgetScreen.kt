...
import com.yash.kagitam.registries.PaperInstanceRegistry.getWidgetInstance
import com.yash.kagitam.db.widgets.WidgetEntity
import com.yash.kagitam.registries.AppRegistry
import com.yash.kagitam.screens.viewModels.WidgetScreenViewModel
...
fun WidgetScreen() {
    ...
    Box(
        ...
        focusWidget.value?.let { widget ->
            ...
            val widInstance = getWidgetInstance(appCtx,widget)
        ...
        if(BuildConfig.IS_DEV_BUILD){
            focusWidgetDev.value?.let{ widget ->
                ...
                val ctx = ContextRegistry.getPluginContext(DevObject.getManifest(AppRegistry.getAppContext()).name)
...
fun WidgetCard(widget: WidgetEntity, onClick: () -> Unit, onLongClick: () -> Unit) {
    val ctx = ContextRegistry.getPluginContext(widget.owner)
    val widInstance = getWidgetInstance(AppRegistry.getAppContext(),widget)
